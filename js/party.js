...
let pendingEpisodeSync = null  // { playlistId, time, playing } — применить после смены эпизода
...
function applySync(data) {
  ...
  if (fileEvent || playlistChanged || fileChanged) {
    ...
    // postMessage 'file' команда не поддерживается SDK — перезагружаем iframe с playlist_id
    const targetPlaylistId = data.file?.playlistId ?? data.playlistId
    currentAudioTrack = null
    switchEpisode(targetPlaylistId, data.time ?? 0, true)
    return  // ждём playlist_changed с нужным ID, потом применяем seek/play
  }
...
// ── Episode switching (iframe src reload) ────────────────────

function switchEpisode(playlistId, seekTime, playing) {
  const frame = document.getElementById('vibix-frame')
  if (!frame) return

  console.log('[party] switchEpisode playlistId:', playlistId)
  playerReady = false
  pendingEpisodeSync = { playlistId, time: seekTime ?? 0, playing: !!playing }

  // Перезагружаем iframe; после ready шлём setCurrentPlaylistItem
  try {
    const url = new URL(frame.src)
    url.searchParams.set('nc', Date.now())
    frame.src = url.toString()
  } catch(e) {
    console.warn('[party] switchEpisode src reload failed:', e.message)
    playerReady = true
    pendingEpisodeSync = null
  }

  // Фолбэк: если через 5с playlist_changed не пришёл — просто play/seek
  setTimeout(() => {
    if (!pendingEpisodeSync) return
    console.warn('[party] episode switch timeout — applying state anyway')
    const { time, playing: p } = pendingEpisodeSync
    pendingEpisodeSync = null
    playerReady = true
...
window.addEventListener('message', e => {
  ...
  if (ev === 'ready' || ev === 'sync_ready') {
    ...
    // После перезагрузки — просим плеер переключить на нужный эпизод
    if (!isHost && pendingEpisodeSync) {
      console.log('[party] player ready, sending setCurrentPlaylistItem:', pendingEpisodeSync.playlistId)
      sendPlayerCommand('setCurrentPlaylistItem', pendingEpisodeSync.playlistId)
    }
    return
  }
  ...
  // Viewer: подтверждение смены эпизода — применяем seek/play
  if (!isHost && pendingEpisodeSync && (ev === 'playlist_changed' || ev === 'file')) {
    const newId = data.playlistId ?? data.file?.playlistId
    console.log('[party] playlist_changed on viewer, newId:', newId, 'target:', pendingEpisodeSync.playlistId)
    if (newId === pendingEpisodeSync.playlistId) {
      const { time, playing } = pendingEpisodeSync
      pendingEpisodeSync = null
      if (time > SYNC_THRESHOLD) sendPlayerCommand('seek', time + latency)
