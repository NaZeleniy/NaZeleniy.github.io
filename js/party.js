...
let pendingEpisodeSync = null  // { time, playing } — применить после перезагрузки iframe
...
function applySync(data) {
  ...
  if (fileEvent || playlistChanged || fileChanged) {
    ...
    switchEpisode(targetPlaylistId, data.time ?? 0)
    return  // ждём ready от плеера, потом применяем seek/play
  }
...
function switchEpisode(playlistId, seekTime) {
  const frame = document.getElementById('vibix-frame')
  if (!frame) return

  console.log('[party] switchEpisode, src:', frame.src)
  playerReady = false
  pendingEpisodeSync = { time: seekTime ?? 0, playing: true }

  try {
    const url = new URL(frame.src)
    url.searchParams.set('playlist_id', playlistId)
    frame.src = url.toString()
    console.log('[party] iframe reloaded with playlist_id:', url.toString())
  } catch(e) {
    console.warn('[party] switchEpisode failed:', e.message)
    playerReady = true
...
window.addEventListener('message', e => {
  ...
  if (ev === 'ready' || ev === 'sync_ready') {
    ...
    // Применяем отложенный seek/play после перезагрузки эпизода
